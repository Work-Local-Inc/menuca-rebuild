...
const getDemoMenus = (restaurantId: string, tenantId: string): RestaurantMenu[] => [
  {
    id: 'menu-demo-1',
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case 'GET':
      ...
      const restaurantMenus = getDemoMenus(restaurantId as string, tenantId);

      return res.status(200).json({
        success: true,
        data: restaurantMenus,
        message: `Found ${restaurantMenus.length} menu(s) for restaurant ${restaurantId}`
      });
