...
export class MenuService {
  ...
  async getRestaurants(tenantId: string, options: PaginationOptions) {
  ...
  async getRestaurantById(tenantId: string, restaurantId: string): Promise<Restaurant | null> {
    const query = `
      SELECT *
      FROM restaurants
      WHERE id = $1
  ...
  async createRestaurant(tenantId: string, ownerId: string, data: Partial<Restaurant>): Promise<Restaurant> {
    const query = `
  ...
  async updateRestaurant(tenantId: string, restaurantId: string, data: Partial<Restaurant>): Promise<Restaurant | null> {
  ...
  async getMenuCategories(tenantId: string, restaurantId: string): Promise<MenuCategory[]> {
    const query = `
      SELECT *
  ...
  async createMenuCategory(tenantId: string, restaurantId: string, data: Partial<MenuCategory>): Promise<MenuCategory> {
    const query = `
  ...
  async getMenuItems(tenantId: string, restaurantId: string, filters: MenuItemFilters = {}): Promise<MenuItem[]> {
  ...
  async getMenuItemById(tenantId: string, restaurantId: string, itemId: string): Promise<MenuItem | null> {
    const query = `
      SELECT mi.*, mc.name as category_name
      FROM menu_items mi
      LEFT JOIN menu_categories mc ON mi.category_id = mc.id
  ...
  async createMenuItem(tenantId: string, restaurantId: string, data: Partial<MenuItem>): Promise<MenuItem> {
    const query = `
  ...
  async updateMenuItem(tenantId: string, restaurantId: string, itemId: string, data: Partial<MenuItem>): Promise<MenuItem | null> {
    const updates: string[] = [];
    const params: any[] = [tenantId, restaurantId, itemId];
    let paramIndex = 4;
  ...
  async deleteMenuItem(tenantId: string, restaurantId: string, itemId: string): Promise<boolean> {
    const query = `
  ...
  private async executeWithTenant(tenantId: string, query: string, params: any[]) {
